...
def get_dashboard():
    ...
    account_ids = [a.id for a in Account.query.filter_by(user_id=DEFAULT_USER_ID).all()]
    ...
    # --- Budget summary ---
    budget_summary = get_budget_summary(DEFAULT_USER_ID, month, year)
    ...
    # --- Unread alerts ---
    unread_alerts = Alert.query.filter_by(user_id=DEFAULT_USER_ID, is_read=False).count()
